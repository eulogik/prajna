...
The three stages use standard objectives applied to the CRN parameters only; the frozen base supplies training signal but receives no gradients. \textbf{SFT} is answer-only masked cross-entropy with a surface-invariance term: each batch contains the original row and a paraphrase-variant row of the same prompt, and the loss
\begin{equation}
\mathcal{L}_{\text{SFT}} = \tfrac{1}{2}\Big[\mathcal{L}_{\text{CE}}(y) + \mathcal{L}_{\text{CE}}(\tilde y) + \lambda\,\mathrm{KL}\big(p^{\text{ans}}_{\theta}(\cdot|x)\,\Vert\,p^{\text{ans}}_{\theta}(\cdot|\tilde x)\big)\Big]
\end{equation}
averages the masked CE on the original ($y$) and variant ($\tilde y$) rows with a small KL consistency term ($\lambda{=}0.1$) between the two answer-start next-token distributions --- a soft pressure to ignore surface form. \textbf{DPO} is reference-free: with $y_c$ the correct completion and $y_r$ the frozen base's wrong completion (rejected),
\begin{equation}
\mathcal{L}_{\text{DPO}} = -\log \sigma\big(\beta [ \log p_\theta(y_c\mid x) - \log p_\theta(y_r\mid x)]\big),
\end{equation}
with $\beta{=}0.1$; there is no external reference model --- the frozen base acts as the data source for rejected completions rather than as a log-probability reference. \textbf{Contrastive} training uses the same ranking form on the correction target,
\begin{equation}
\mathcal{L}_{\text{CON}} = -\log \sigma\big(\beta [ \log p_\theta(y_c\mid x) - \log p_\theta(y_w\mid x)]\big),
\end{equation}
where $y_w$ is the base's wrong output; this stage refines the reflective loop to prefer correction directions that raise the correct completion above the wrong one. Memories are written every step via a learned gate ($m_l$ in Eq.~\ref{eq:inject} is distinct from the memory write gate).
...
\textbf{Data.} 16{,}680 prompt$\to$answer base pairs across three domains (arithmetic, facts, implicit-goal reasoning), each expanded to 5 rows (the original plus 4 paraphrase variants) --- 83{,}400 rows (Table~\ref{tab:data}); the 17{,}810 unique prompt texts become the retrieval-table keys (Section~\ref{sec:retrievalgate}). The 60-question CEHRI exam is drawn from the training distribution (its prompts appear verbatim in the corpus; exact-recall similarity 1.000), so its 100\% reflects exact recall of seen prompts; the reworded exam (120 items) applies a second, template-based paraphrase transform of the same prompts and serves as the near-duplicate test (Section~\ref{sec:results}).

\textbf{Stages.} (i) \emph{SFT}: the surface-invariance objective above, LR $3\times10^{-4}$, weight decay 0, cosine annealing to $0.1\times$. (ii) \emph{DPO}: LR $5\times10^{-6}$, $\beta{=}0.1$, preferring the correct completion over the frozen base's wrong one. (iii) \emph{Contrastive}: LR $5\times10^{-6}$, ranking correct completions above the base's wrong ones to refine the reflective loop. Step budgets are environment-configurable (committed defaults: 2{,}000 SFT / 500 DPO / 200 contrastive; experimental stage runs used up to 3{,}000/3{,}000/1{,}000). The released seed checkpoint's internal step counter reads 3{,}000; because the trainer reuses a single output filename across stages, per-stage provenance within the released seed is imperfectly recorded by filenames alone --- we therefore release the exact checkpoint bytes together with the trainer (Table~\ref{tab:budget}). Checkpoints every 50 steps with resumable state.

\textbf{Hardware.} Apple M4 (Mac Mini, 16\,GB RAM). Training runs on CPU (MPS is unusable for the wrapped model); evaluation runs on MPS. Measured throughput: $\sim$0.75\,s/step for DPO/contrastive and $\sim$2\,s/step for SFT; each observed stage run completes in under two hours on the single consumer machine (Table~\ref{tab:budget}).

\begin{table}[ht]
\centering
\small
\caption{Observed on-device stage runs (Apple M4, CPU; timings derived from checkpoint timestamps). Step budgets are env-configurable; these are the recorded experimental runs.}
\begin{tabular}{@{}lrrc@{}}
\toprule
Stage & Steps & Observed time & Base \\
\midrule
SFT  & 3{,}000 & $\sim$1.75\,h & frozen \\
DPO  & 3{,}000 & $\sim$37\,min & frozen \\
Contrastive & 1{,}000 & $\sim$12\,min & frozen \\
\bottomrule
\end{tabular}
\label{tab:budget}
\end{table}
...
\caption{The answer token is already recoverable at position $L{-}2$ (top-1 = the correct answer, consistent across probe prompts); at the masked answer-start position $L{-}1$ the base emits formatting/digit tokens (top-1 = digit / \texttt{<strong>}). \texttt{GEN\_CLAMP} emits the $L{-}2$ argmax, harvesting this knowledge.}
\end{figure}
...
Table~\ref{tab:ood} summarizes out-of-distribution behavior. The implicit-goal (``car-wash'') check deserves emphasis: an earlier evaluation counted prompt-echo matches as successes; rescoring honestly gives 3/8 on original phrasing (keyword luck) and \textbf{0/8} on reworded phrasing. The CRN memorizes surface patterns and does not perform latent-goal reasoning.
...
\subsection{Mechanism: absent supervision and an attenuating frozen readout}

The untrained gates and the boundary-position failure share a common cause, which we state as a hypothesis. First, the boundary position $L{-}1$ receives \emph{no direct supervision}: it is masked in SFT, so no gradient ever teaches the corrections to place answer mass there. Second, the correction must act through the frozen LM head, whose fixed readout attenuates small hidden-state perturbations when the base's predictive distribution is diffuse; empirically, correction gradients measured on the converged seed were near zero (the ``dry-signal'' regime that aborted the rank-4 run). Together these explain: (i) why the boundary position $L{-}1$ never moved (three training strategies failed); (ii) why logit-fusion rank 4 produced near-zero gradients; (iii) why the per-depth gates did not learn to differentiate depths; and (iv) why GEN\_CLAMP --- which bypasses training entirely and reads the $L{-}2$ logits directly --- is the only reliable fix. The CRN's additive corrections at mid-depths (layers 7/15) do learn, because the base's representations at those depths carry stronger task-specific signal; the final-layer boundary position does not. Direct gradient-norm measurements across depths and stages (Section~\ref{sec:negative}, Future Work) would confirm the hypothesis quantitatively.
...
\caption{Benchmark snapshot. Blue and green cards are the released seed checkpoint (memory-augmented: 60/60 and 110/120); gray cards are generation-only results for the released seed (12/60, 30/120) with the frozen base for comparison (7/60); orange and red cards are the experimental anchor-trained checkpoint whose generation had collapsed, showing GEN\_CLAMP recovery (19/60 and 41/120). All numbers are produced by committed scripts with the released checkpoint.}
\end{figure}
